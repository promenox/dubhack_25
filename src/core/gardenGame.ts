...
export class GardenGame {
  ...
  private constructor(
    private readonly storage: GardenStorage,
  ...
  static async create(storage: GardenStorage): Promise<GardenGame> {
    const stored = await storage
      .load()
      .catch(() => createInitialGardenState());
  ...
  addCurrency(amount: number) {
    this.state.inventory.currency = Math.max(
      0,
      this.state.inventory.currency + amount
    );
  }
  ...
  async buySeed(seedType: PlantType) {
    ...
    this.state.inventory.seeds[seedType] =
      (this.state.inventory.seeds[seedType] ?? 0) + 1;
